...
import codexDesktopBridgeService, { buildCodexDesktopBridgeMetadata, buildCodexDesktopBridgeSessionPatch } from '../codex-desktop-bridge.js';
...
function latestTurnIdForSession(sessionId) {
  return agentRuntimeSessionManager.listTurns(String(sessionId || ''), { limit: 1 })[0]?.id || '';
}
...
export async function handleBridgeAgentRuntimeSessionToCodexDesktop(req, res) {
  const sessionId = String(req.params.id || '');
  const session = agentRuntimeSessionManager.getSession(sessionId);
  if (!session) {
    return res.status(404).json({ success: false, error: 'session not found' });
  ...
  try {
    const bridge = await codexDesktopBridgeService.bridgeRuntimeSession({
      session,
      openApp: req.body?.openApp !== false
    });
    const updatedSession = agentRuntimeSessionManager.updateSession(session.id, buildCodexDesktopBridgeSessionPatch(session, bridge, {
        lastBridgedTurnId: latestTurnIdForSession(session.id),
        autoBridged: false
      }));
    return res.json({
      success: true,
    ...
    try {
      agentRuntimeSessionManager.updateSessionMetadata(session.id, {
      ...
      // Ignore metadata write failures while reporting the bridge error.
    }

    const status = /not found/i.test(error.message)
      ? 404
      : (/bridgeable|timed out|failed to bridge/i.test(error.message) ? 400 : 500);
    return res.status(status).json({
      success: false,
      error: error.message
    });
