...
export function buildCodexSpawnEnv(baseEnv = process.env, runtimeOptions = {}, { platform = process.platform } = {}) {
  ...
  if (
    ...
    && env[pathKey]
    && parseBooleanFlag(env.REMOTECODEX_CODEX_FORCE_WINDOWS_POWERSHELL ?? '1')
  ) {
...
export function resolveCodexRuntimeOptions(session, { env = process.env } = {}) {
  ...
  const dangerouslyBypass = parseBooleanFlag(
    codexOptions.dangerouslyBypass
      ?? runtimeOptions.dangerouslyBypass
      ?? env.REMOTECODEX_CODEX_DANGEROUSLY_BYPASS
  );
  ...
  const sandboxSource = readRuntimeOption(
    ...
    'sandbox_mode',
    'sandbox'
  ) || readRuntimeOption(runtimeOptions, 'sandboxMode', 'sandbox_mode', 'sandbox') || env.REMOTECODEX_CODEX_SANDBOX_MODE;

  const approvalSource = readRuntimeOption(
    codexOptions,
    'approvalPolicy',
    'approval_policy'
  ) || readRuntimeOption(runtimeOptions, 'approvalPolicy', 'approval_policy') || env.REMOTECODEX_CODEX_APPROVAL_POLICY;
...
function extractCodexImagePaths(inputAttachments = []) {
  if (!Array.isArray(inputAttachments)) {
    return [];
  }

  return inputAttachments
    .filter((attachment) => String(attachment?.kind || '').toLowerCase() === 'image')
    .map((attachment) => String(attachment?.localPath || attachment?.path || '').trim())
    .filter(Boolean);
}

export function buildCodexExecArgs(session, input = '', { env = process.env, inputAttachments = [] } = {}) {
  ...
  for (const imagePath of extractCodexImagePaths(inputAttachments)) {
...
export class CodexProvider {
  ...
  async startTurn({ session, input, inputAttachments = [], onProviderEvent, onApprovalRequest, onQuestionRequest, onSessionPatch, onTurnFinished, onTurnFailed }) {
