...
export class AgentRuntimeSessionManager {
  ...
  listSessionsByConversationId(conversationId, { limit = 50 } = {}) {
  ...
  updateSession(sessionId, patch = {}) {
    const session = this.getSession(String(sessionId || ''));
    if (!session) {
      throw new Error('session not found');
    }

    const nextPatch = { ...(patch || {}) };
    if (Object.prototype.hasOwnProperty.call(nextPatch, 'metadata')) {
      nextPatch.metadata = {
        ...(session.metadata || {}),
        ...(nextPatch.metadata || {})
      };
    ...
    return this._patchSession(session.id, nextPatch);
  }

  updateSessionMetadata(sessionId, metadataPatch = {}) {
  ...
  async createSession({ provider, input, cwd, model = '', metadata = {}, inputAttachments = [] } = {}) {
  ...
  async sendInput(sessionId, input, { inputAttachments = [] } = {}) {
    const session = this.getSession(sessionId);
    if (!session) {
      throw new Error('session not found');
    }
  ...
  cancelSession(sessionId) {
    ...
    const handle = this.turnHandles.get(sessionId);
    const activeTurnId = String(session.currentTurnId || '').trim();
    ...
    if (activeTurnId) {
      this._patchTurn(sessionId, activeTurnId, {
  ...
  async _startTurn(sessionId, input, { inputAttachments = [] } = {}) {
    const session = this.getSession(sessionId);
    if (!session) {
      throw new Error('session not found');
    }
    ...
    const isSessionCancelled = () => (
      ...
      || this.getSession(sessionId)?.status === AGENT_SESSION_STATUS.CANCELLED
    );
